...
private:
    ...
    // NEW HELPER: Traverses the BST to collect all student nodes into a vector
    void getAllStudentsRecursive(StudentNode* node, vector<StudentNode*>& allStudents) {
        if (node == nullptr) {
            return;
        }
        getAllStudentsRecursive(node->left, allStudents);
        allStudents.push_back(node); // Add current node to the vector
        getAllStudentsRecursive(node->right, allStudents);
    }
...
public:
    ...
    // NEW: Public method to get all students (for linear search in main)
    vector<StudentNode*> getAllStudents() {
        vector<StudentNode*> allStudents;
        getAllStudentsRecursive(root, allStudents);
        return allStudents;
    }
};
...
int main() {
    ...
    bst.insert(20241110350, "Paul Wesley Gomez", "gomez.paulwesley@ue.edu.ph", "1CPE-2A", 98);
